...
int basic_to_asm(const char *filename_bas, const char *filename_asm)
{
    ...
    while (end == 0 && fscanf(fbas, "%u %[A-Z] ", &line, buffer) != 0)
    {
        ...
        { // проверка что строки идут по возрастанию
            end = fail = 1;
            break;
        }
    ...
    type_check:
        ...
        case 4:
        {
            ...
            strcat(asm_code, buffer);
            fscanf(fbas, "%u", &line);
            if (line > lines[line_id - 1].line_number) // 70 > 20 && 30 > 50 // запоминаем позицию для адреса
            {
                _goto[goto_id].goto_line = line;
                _goto[goto_id].instratuction_address = strlen(asm_code); // 60
                strcat(asm_code, "00\n");
                ++goto_id;
            ...
            int id = line_id - 1;
            while (id >= 0 && line != lines[id].line_number) // ищем стартовый адрес строки в ассемблере
                --id;
            if (id < 0)
            {
                end = fail = 1;
        ...
        case 5:
        {
            ...
            default:
                ...
                sprintf(buffer, "%02u LOAD %02u\n%02u SUB %02u\n%02u JZ %02u\n%02u JUMP 00\n",
                        ...
                        address + 3);
                address += 4;
                _goto[goto_id].goto_line = line + 1; // 31
                _goto[goto_id].instratuction_address = strlen(asm_code) + 38; // 49
                ++goto_id;
                break;
            }
            strcat(asm_code, buffer);
